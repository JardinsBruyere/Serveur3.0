...
def capteur():
    try:
        ...
        if values[1] is None:                               # s'il y a aucun parametre, on prend 4 comme valeur par default
            args2 = 'hours'
        else:
            args2 = str(values[1])
        #print(args2)
	print("----------Requete de donnée pour "+str(args1)+" "+args2+"----------")
        timeLine= str((currentTimestamp-timedelta(**{args2:args1})).strftime("%Y-%m-%d %H:%M:%S"))     # date d'aujourd'hui - nb semaines
...
@app.route('/')
def hello():
    #return "If you have any trouble with this server please contact us with this link\nshorturl.at/cinKY"
